...
export const pipe = (fn: (firstNode: Node, secondNode: Node) => Node, firstNode: Node, ...moreNodes: Node[]) => {
  return moreNodes.reduce((acc, node) => fn(acc, node), firstNode)
}

export const blendAlpha = (a: Node, b: Node) => mix(a, b, b.w)

export const multiplyRgbByAlpha = (color: Node | UniformNode<unknown>) => vec4(color.xyz.mul(color.w), color.w)
