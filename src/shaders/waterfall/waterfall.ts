...
type WaterfallOptions = {
  ...
  // River noise scales
  riverDeepBlueScale?: number
  riverFlowScale?: number
  riverEdgeFoamScale?: number
  riverSurfaceFoamScale?: number

  // Fall noise scales
  fallFoamScale1?: number
  fallFoamScale2?: number
  fallFoamScale3?: number
  fallFoamScale4?: number
  fallFoamScale5?: number
  fallHorizontalScale?: number
}

const sharedFoamColor = '#8be'
...
const defaultOptions: Required<WaterfallOptions> = {
  ...
  // River noise scales
  riverDeepBlueScale: 3.0,
  riverFlowScale: 3.0,
  riverEdgeFoamScale: 20.0,
  riverSurfaceFoamScale: 12.0,
...
const createRiver = (options: Required<WaterfallOptions>) => {
  ...
  // River flow towards bottom - parametrized scale
  const flowNoise = mx_fractal_noise_vec3(vec3(mul(uvCoord.x, options.riverFlowScale), sub(uvCoord.y, timeOffset), 0)).x
  ...
  // Additional surface foam - parametrized scale
...
const createFall = (options: Required<WaterfallOptions>) => {
  ...
  // Multiple layers of vertical foam lines - all with parametrized scales
  const verticalFlow = add(uvCoord.y, timeOffset)

  // Primary foam lines - parametrized scale
  const foamLines1 = mx_fractal_noise_vec3(vec3(mul(uvCoord.x, options.fallFoamScale1), verticalFlow, 0)).x
  const foamMask1 = step(0.5, foamLines1)

  // Secondary foam lines - parametrized scale
  const foamLines2 = mx_fractal_noise_vec3(vec3(mul(uvCoord.x, options.fallFoamScale2), mul(verticalFlow, 1.2), 1)).x
  const foamMask2 = step(0.6, foamLines2)

  // Tertiary foam lines - parametrized scale
  const foamLines3 = mx_noise_vec3(vec3(mul(uvCoord.x, options.fallFoamScale3), mul(verticalFlow, 0.8), 2)).x
  const foamMask3 = step(0.55, foamLines3)

  // Fourth layer - parametrized scale
  const foamLines4 = mx_fractal_noise_vec3(vec3(mul(uvCoord.x, options.fallFoamScale4), mul(verticalFlow, 1.4), 3)).x
  const foamMask4 = step(0.72, foamLines4)

  // Fifth layer - parametrized scale
  const foamLines5 = mx_noise_vec3(vec3(mul(uvCoord.x, options.fallFoamScale5), mul(verticalFlow, 0.9), 4)).x
